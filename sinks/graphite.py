...
class GraphiteStore(object):
    def __init__(self, host="localhost", port=2003, prefix="statsite.", attempts=3, protocol='lines'):
        ...
        if protocol not in ["pickled", "lines"]:
            raise ValueError("Supported protocols are pickled, lines")
        ...
        self.flush = self.flush_pickled if protocol == "pickled" else self.flush_lines
    ...
    def flush_pickled(self, metrics):
        """
        Flushes the metrics provided to Graphite.
       ...
       :Parameters:
        ...
        # pickled graphite interface supports, in the form of
        # (key, (timestamp, value))
